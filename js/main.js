    ...
    function loadMaterialsForGrade(grade) {
        ...
        setTimeout(() => {
            ...
            const gradeContent = {
                ...
                '11': {
                    'ngabasa': 'Kaidah basa Sunda, éjahan, jeung paguneman tingkat menengah',
                ...
                '12': {
                    ...
                    'sastra': 'Kritik sastra, drama, jeung sajak modern Sunda tingkat lanjut',
            ...
            categories.forEach(category => {
                ...
                if (descriptionElement && gradeContent[grade] && gradeContent[grade][categoryType]) {
    ...
    function setupMultipleChoiceEvents() {
        ...
        if (checkBtn && result) {
            checkBtn.addEventListener('click', function() {
                ...
                if (!selectedOption) {
                    result.innerHTML = '<div class="error">Pilih jawaban terlebih dahulu!</div>';
                    return;
                }
                ...
                if (answer === 'B') {
                    result.innerHTML = '<div class="success">Benar! "Someah" artinya "Ramah".</div>';
                    
                    // Add score to user progress
                    updateUserProgress('quiz', 10);
                } else {
                    result.innerHTML = '<div class="error">Salah! Jawaban yang benar adalah B. Ramah.</div>';
                }
            });
        }
    }
    ...
    function setupMatchingEvents() {
        ...
        if (checkBtn && result) {
            checkBtn.addEventListener('click', function() {
                ...
                if (correct) {
                    result.innerHTML = '<div class="success">Benar semua! Anda telah menjodohkan dengan benar.</div>';
                    
                    // Add score to user progress
                    updateUserProgress('matching', 15);
                } else {
                    result.innerHTML = '<div class="error">Masih ada yang salah. Coba lagi!</div>';
                }
            });
        }
    }
        ...
        if (checkBtn && result) {
            checkBtn.addEventListener('click', function() {
                ...
                if (sentence === correctSentence) {
                    result.innerHTML = '<div class="success">Benar! Kalimat tersusun dengan tepat.</div>';
...
function initProgressChart() {
    ...
    if (ctx) {
        ...
        if (typeof Chart !== 'undefined') {
            ...
            ctx.innerHTML = '<div class="chart-fallback">Progress chart menampilkan skor latihan dalam 5 kategori berbeda.</div>';
        }
    }
}
...
function updateUserProgress(activity, points) {
    ...
    // Update counters
    if (activity === 'quiz' || activity === 'matching' || activity === 'sentence') {
    ...
    // Calculate badges
    const totalScore = Object.values(progress.scores).reduce((total, score) => total + score, 0);
